...
/**
 * Class represents a bloockcert template.
 *
...
class template {
    ...
    public function save($data) {
        ...
        $DB->update_record('bloockcert_templates', $savedata);

        \mod_bloockcert\event\template_updated::create_from_template($this)->trigger();
    }
    ...
    public function add_page() {
        ...
        // Insert the page.
        $pageid = $DB->insert_record('bloockcert_pages', $page);

        $page->id = $pageid;

        \mod_bloockcert\event\page_created::create_from_page($page, $this)->trigger();
        \mod_bloockcert\event\template_updated::create_from_template($this)->trigger();
    ...
    public function save_page($data) {
        ...
        if ($pages = $DB->get_records('bloockcert_pages', array('templateid' => $data->tid))) {
            ...
            foreach ($pages as $page) {
                ...
                $p->timemodified = $time;
                // Update the page.
                $DB->update_record('bloockcert_pages', $p);

                \mod_bloockcert\event\page_updated::create_from_page($p, $this)->trigger();
            }
        ...
        if ($elements = $DB->get_records_sql($sql, array('templateid' => $this->id))) {
            foreach ($elements as $element) {
                ...
                if ($e = \mod_bloockcert\element_factory::get_element_instance($element)) {
                    ...
                    $DB->delete_records('bloockcert_elements', array('id' => $element->id));
                }
            }
        }

        // Delete the pages.
        if (!$DB->delete_records('bloockcert_pages', array('templateid' => $this->id))) {
            return false;
        }

        // Now, finally delete the actual template.
        if (!$DB->delete_records('bloockcert_templates', array('id' => $this->id))) {
            return false;
        }

        \mod_bloockcert\event\template_deleted::create_from_template($this)->trigger();
    ...
    public function delete_page($pageid) {
        ...
        // Get the page.
        $page = $DB->get_record('bloockcert_pages', array('id' => $pageid), '*', MUST_EXIST);

        // Delete this page.
        $DB->delete_records('bloockcert_pages', array('id' => $page->id));

        \mod_bloockcert\event\page_deleted::create_from_page($page, $this)->trigger();

        // The element may have some extra tasks it needs to complete to completely delete itself.
        if ($elements = $DB->get_records('bloockcert_elements', array('pageid' => $page->id))) {
            foreach ($elements as $element) {
                // Get an instance of the element class.
                if ($e = \mod_bloockcert\element_factory::get_element_instance($element)) {
                    $e->delete();
    ...
    public function delete_element($elementid) {
        ...
        $element = $DB->get_record('bloockcert_elements', array('id' => $elementid), '*', MUST_EXIST);
    ...
    public function generate_pdf(bool $preview = false, int $userid = null, bool $return = false) {
        ...
        if ($pages = $DB->get_records('bloockcert_pages', array('templateid' => $this->id), 'sequence ASC')) {
            // Create the pdf object.
            $pdf = new \pdf();

            $bloockcert = $DB->get_record('bloockcert', ['templateid' => $this->id]);

            // I want to have my digital diplomas without having to change my preferred language.
            $userlang = $USER->lang ?? current_language();

            // Check the $bloockcert exists as it is false when previewing from mod/bloockcert/manage_templates.php.
            if ($bloockcert) {
                $forcelang = mod_bloockcert_force_current_language($bloockcert->language);
                if (!empty($forcelang)) {
            ...
            if (!empty($bloockcert->protection)) {
                $protection = explode(', ', $bloockcert->protection);
                $pdf->SetProtection($protection);
            }

            if (empty($bloockcert->deliveryoption)) {
                $deliveryoption = certificate::DELIVERY_OPTION_INLINE;
            } else {
                $deliveryoption = $bloockcert->deliveryoption;
            }
            ...
            if (empty($filename)) {
                $filename = get_string('certificate', 'bloockcert');
            }
            ...
            if ($bloockcert) {
                ...
                if ($userlang != $bloockcert->language) {
                    mod_bloockcert_force_current_language($userlang);
                }
            }
     ...
     * @param bool $preview true if it is a preview, false otherwise
     * @param int $userid the id of the user whose certificate we want to view
     * @param int $bloockcertid the id of the certificate we want to create
     * @param bool $return Do we want to return the contents of the PDF?
     * @return string|void Can return the PDF in string format if specified.
     */
    public function save_pdf(bool $preview = false, int $userid = null, int $bloockcertid = null, bool $return = false) {
        global $CFG, $DB, $USER, $COURSE;

        if ($bloockcertid !== null) {
            $certificateid = $bloockcertid;
        } else {
            $certificateid = 0;
        }
        ...
        $certificate = $DB->get_record('bloockcert_issues',
        ['userid' => $userid, 'bloockcertid' => $certificateid],
        'recordid', MUST_EXIST);

        $id = $certificate->recordid;

        if ($id != null) {
            header("Location: https://cdn.bloock.com/hosting/ipfs/$id");
            exit;
        }
        ...
        if ($pages = $DB->get_records('bloockcert_pages', array('templateid' => $this->id), 'sequence ASC')) {
            ...
            // Remove full-stop at the end, if it exists, to avoid "..pdf" being created and being filtered by clean_filename.
            $filename = rtrim(format_string("Bloockcert_" . $user->id . $COURSE->id . $this->id,
            true, ['context' => $this->get_context()]), '.');
            ...
            // Save the PDF into server.
            $pdffilepath = __DIR__ . "/certificates/" . $filename;
            $pdf->Output($pdffilepath, 'F');

            $this->sign_pdf($pdffilepath, $user->id, $certificateid);
            exit;
        }
    }
     ...
     * @param int $userid the id of the user for whom the certificate will be generated.
     * @param int $certificateid the id of the certificate to be generated.
     * @return void Return void.
     */
    public function sign_pdf(String $filename, int $userid, int $certificateid) {
        global $DB;
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $urlclient.'/v1/process',
    ...
    public function copy_to_template($copytotemplate) {
        ...
        // Get the pages for the template, there should always be at least one page for each template.
        if ($templatepages = $DB->get_records('bloockcert_pages', array('templateid' => $this->id))) {
            ...
            foreach ($templatepages as $templatepage) {
                ...
                $page->id = $DB->insert_record('bloockcert_pages', $page);
                \mod_bloockcert\event\page_created::create_from_page($page, $this)->trigger();
                // Now go through the elements we want to load.
                if ($templateelements = $DB->get_records('bloockcert_elements', array('pageid' => $templatepage->id))) {
                    foreach ($templateelements as $templateelement) {
                        ...
                        if ($e = \mod_bloockcert\element_factory::get_element_instance($element)) {
                            if (!$e->copy_element($templateelement)) {
                                ...
                                \mod_bloockcert\event\element_created::create_from_element($e)->trigger();
                            }
                        }
                    }
                }
            ...
            if ($copytotemplate->get_context() == \context_system::instance()) {
                ...
                \mod_bloockcert\event\template_created::create_from_template($copytotemplate)->trigger();
            } else {
                // Otherwise we're loading template in a course module instance.
                \mod_bloockcert\event\template_updated::create_from_template($copytotemplate)->trigger();
            }
        }
    }
    ...
    public function get_cm() {
        ...
        if ($context->contextlevel === CONTEXT_MODULE) {
            return get_coursemodule_from_id('bloockcert', $context->instanceid, 0, false, MUST_EXIST);
        }
    ...
    public function require_manage() {
        require_capability('mod/bloockcert:manage', $this->get_context());
    }
     ...
     * @return \mod_bloockcert\template the template object
     */
    public static function create($templatename, $contextid) {
        ...
        $template->id = $DB->insert_record('bloockcert_templates', $template);

        $template = new \mod_bloockcert\template($template);

        \mod_bloockcert\event\template_created::create_from_template($template)->trigger();
