...
class element_helper {
     ...
     */
    const BLOOCKCERT_REF_POINT_TOPLEFT = 0;

    /**
     * @var int the top-center of element
     */
    const BLOOCKCERT_REF_POINT_TOPCENTER = 1;

    /**
     * @var int the top-left of element
     */
    const BLOOCKCERT_REF_POINT_TOPRIGHT = 2;

    /**
     * Common behaviour for rendering specified content on the pdf.
     *
     * @param \pdf $pdf the pdf object
     * @param \mod_bloockcert\element $element the bloockcert element
    ...
    public static function render_content($pdf, $element, $content) {
        ...
        switch ($refpoint) {
            case self::BLOOCKCERT_REF_POINT_TOPRIGHT:
                ...
                break;
            case self::BLOOCKCERT_REF_POINT_TOPCENTER:
    ...
    public static function render_form_element_font($mform) {
        $mform->addElement('select', 'font', get_string('font', 'bloockcert'), \mod_bloockcert\certificate::get_fonts());
        $mform->setType('font', PARAM_TEXT);
        $mform->setDefault('font', 'times');
        $mform->addHelpButton('font', 'font', 'bloockcert');
        $mform->addElement('select', 'fontsize', get_string('fontsize', 'bloockcert'),
            \mod_bloockcert\certificate::get_font_sizes());
        $mform->setType('fontsize', PARAM_INT);
        $mform->setDefault('fontsize', 12);
        $mform->addHelpButton('fontsize', 'fontsize', 'bloockcert');
    }
    ...
    public static function render_form_element_colour($mform) {
        $mform->addElement('bloockcert_colourpicker', 'colour', get_string('fontcolour', 'bloockcert'));
        $mform->setType('colour', PARAM_RAW); // Need to validate that this is a valid colour.
        $mform->setDefault('colour', '#000000');
        $mform->addHelpButton('colour', 'fontcolour', 'bloockcert');
    }
    ...
    public static function render_form_element_position($mform) {
        $mform->addElement('text', 'posx', get_string('posx', 'bloockcert'), array('size' => 10));
        $mform->setType('posx', PARAM_INT);
        $mform->setDefault('posx', 0);
        $mform->addHelpButton('posx', 'posx', 'bloockcert');
        $mform->addElement('text', 'posy', get_string('posy', 'bloockcert'), array('size' => 10));
        $mform->setType('posy', PARAM_INT);
        $mform->setDefault('posy', 0);
        $mform->addHelpButton('posy', 'posy', 'bloockcert');
    }
    ...
    public static function render_form_element_width($mform) {
        $mform->addElement('text', 'width', get_string('elementwidth', 'bloockcert'), array('size' => 10));
        $mform->setType('width', PARAM_INT);
        $mform->setDefault('width', 0);
        $mform->addHelpButton('width', 'elementwidth', 'bloockcert');
    }
    ...
    public static function render_form_element_height($mform) {
        $mform->addElement('text', 'height', get_string('elementheight', 'bloockcert'), array('size' => 10));
        $mform->setType('height', PARAM_INT);
        $mform->setDefault('height', 0);
        $mform->addHelpButton('height', 'elementheight', 'bloockcert');
    }
    ...
    public static function render_form_element_refpoint($mform) {
        ...
        $refpointoptions[self::BLOOCKCERT_REF_POINT_TOPLEFT] = get_string('topleft', 'bloockcert');
        $refpointoptions[self::BLOOCKCERT_REF_POINT_TOPCENTER] = get_string('topcenter', 'bloockcert');
        $refpointoptions[self::BLOOCKCERT_REF_POINT_TOPRIGHT] = get_string('topright', 'bloockcert');

        $mform->addElement('select', 'refpoint', get_string('refpoint', 'bloockcert'), $refpointoptions);
        $mform->setType('refpoint', PARAM_INT);
        $mform->setDefault('refpoint', self::BLOOCKCERT_REF_POINT_TOPCENTER);
        $mform->addHelpButton('refpoint', 'refpoint', 'bloockcert');
    }
    ...
    public static function render_form_element_alignment($mform) {
        ...
        $alignmentoptions[element::ALIGN_LEFT] = get_string('alignleft', 'bloockcert');
        $alignmentoptions[element::ALIGN_CENTER] = get_string('aligncenter', 'bloockcert');
        $alignmentoptions[element::ALIGN_RIGHT] = get_string('alignright', 'bloockcert');

        $mform->addElement('select', 'alignment', get_string('alignment', 'bloockcert'), $alignmentoptions);
        $mform->setType('alignment', PARAM_ALPHA);
        $mform->setDefault('alignment', element::ALIGN_LEFT);
        $mform->addHelpButton('alignment', 'alignment', 'bloockcert');
    }
    ...
    public static function validate_form_element_colour($data) {
        ...
        if (!self::validate_colour($data['colour'])) {
            $errors['colour'] = get_string('invalidcolour', 'bloockcert');
        }
        return $errors;
    }
    ...
    public static function validate_form_element_width($data, bool $allowzero = true) {
        ...
        if (!is_numeric($data['width'])) {
            $errors['width'] = get_string('invalidelementwidthorheightnotnumber', 'bloockcert');
        } else {
            if ($allowzero) {
                if ($data['width'] < 0) {
                    $errors['width'] = get_string('invalidelementwidthorheightzeroallowed', 'bloockcert');
                }
            } else {
                if ($data['width'] <= 0) {
                    $errors['width'] = get_string('invalidelementwidthorheightzeronotallowed', 'bloockcert');
                }
            }
        }
    ...
    public static function validate_form_element_height($data, bool $allowzero = true) {
        ...
        if (!is_numeric($data['height'])) {
            $errors['height'] = get_string('invalidelementwidthorheightnotnumber', 'bloockcert');
        } else {
            if ($allowzero) {
                if ($data['height'] < 0) {
                    $errors['height'] = get_string('invalidelementwidthorheightzeroallowed', 'bloockcert');
                }
            } else {
                if ($data['height'] <= 0) {
                    $errors['height'] = get_string('invalidelementwidthorheightzeronotallowed', 'bloockcert');
                }
            }
        }
    ...
    /**
     * Helper function that returns the sequence on a specified bloockcert page for a
    ...
    public static function get_courseid($elementid) {
        ...
        $sql = "SELECT course
                  FROM {bloockcert} c
            INNER JOIN {bloockcert_pages} cp
                    ON c.templateid = cp.templateid
            INNER JOIN {bloockcert_elements} ce
                    ON cp.id = ce.pageid
    ...
    public static function get_available_element_types() {
        ...
        $elementdir = "$CFG->dirroot/mod/bloockcert/element";
        ...
        if ($gradeitems = \grade_item::fetch_all(['courseid' => $course->id])) {
            foreach ($gradeitems as $gi) {
                ...
                if ($gi->is_external_item() && !$gi->is_outcome_item()) {
                    ...
                    $arrgradeitems[$cm->id] = get_string('activity', 'mod_bloockcert') . ' : ' . $gi->get_name();
                } else if ($gi->is_external_item() && $gi->is_outcome_item()) {
                    // Get the name of the activity.
                    $optionname = get_string('gradeoutcome', 'mod_bloockcert') . ' : '  . $modname . " - " . $gi->get_name();
