...
class external extends \external_api {
    ...
    public static function save_element($templateid, $elementid, $values) {
        ...
        $template = $DB->get_record('bloockcert_templates', array('id' => $templateid), '*', MUST_EXIST);
    ...
    /**
     * Handles deleting a bloockcert issue.
     *
    ...
    public static function delete_issue($certificateid, $issueid) {
        ...
        $issue = $DB->get_record('bloockcert_issues', ['id' => $issueid, 'bloockcertid' => $certificateid], '*', MUST_EXIST);

        $cm = get_coursemodule_from_instance('bloockcert', $certificate->id, 0, false, MUST_EXIST);
        ...
        // Delete the issue.
        return $DB->delete_records('bloockcert_issues', ['id' => $issue->id]);
    }
