...
namespace mod_bloockcert\output;
...
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('bloockcert', $cmid);
        $context = \context_module::instance($cm->id);
        self::require_capability($cm, $context, 'mod/bloockcert:view');

        // Set some variables we are going to be using.
        $certificate = $DB->get_record('bloockcert', ['id' => $cm->instance], '*', MUST_EXIST);
        ...
        list($certificate->intro, $certificate->introformat) = external_format_text($certificate->intro,
            $certificate->introformat, $context->id, 'mod_bloockcert', 'intro');

        // Get any issues this person may have.
        $issue = false;
        if ($issues = $DB->get_records('bloockcert_issues', ['userid' => $USER->id, 'bloockcertid' => $certificate->id],
                'timecreated DESC')) {
            $issue = reset($issues);
        }
        ...
        $fileurl = "";
        if ($requiredtimemet) {
            $fileurl = new \moodle_url('/mod/bloockcert/mobile/pluginfile.php', ['certificateid' => $certificate->id,
                'userid' => $USER->id]);
        ...
        $groups = [];
        $recipients = [];
        if (has_capability('mod/bloockcert:viewreport', $context)) {
            $showreport = true;
            ...
            $recipients = \mod_bloockcert\certificate::get_issues($certificate->id, $groupmode, $cm, 0, 0);
            foreach ($recipients as $recipient) {
                $recipient->displayname = fullname($recipient);
                $recipient->fileurl = new \moodle_url('/mod/bloockcert/mobile/pluginfile.php', ['certificateid' => $certificate->id,
                    'userid' => $recipient->id]);
            }
        }
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template("mod_bloockcert/mobile_view_activity_page_$versionname", $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => [
