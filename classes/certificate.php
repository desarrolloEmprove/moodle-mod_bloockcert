...
/**
 * Provides functionality needed by bloockcert activities.
 *
...
class certificate {
     ...
     *      If you want to display all bloockcerts on a page set this to 0.
     */
    const BLOOCKCERT_PER_PAGE = '50';

    /**
     * Handles setting the protection field for the bloockcert
     *
     * @param \stdClass $data
    ...
    /**
     * Handles uploading an image for the bloockcert module.
     *
    ...
    public static function upload_files($draftitemid, $contextid, $filearea = 'image') {
        ...
        file_save_draft_area_files($draftitemid, $contextid, 'mod_bloockcert', $filearea, 0);
    }
    ...
    /**
     * Returns a list of issued bloockcerts.
     *
     ...
     */
    public static function get_issues($bloockcertid, $groupmode, $cm, $limitfrom, $limitnum, $sort = '') {
        global $DB;
        ...
        // Add the conditional SQL and the bloockcertid to form all used parameters.
        $allparams = $query->params + $conditionsparams + array('bloockcertid' => $bloockcertid);

        $orderby = $sort ?: $DB->sql_fullname();

        $sql = "SELECT $query->selects, ci.id as issueid, ci.code, ci.timecreated
                  FROM {user} u
            INNER JOIN {bloockcert_issues} ci ON (u.id = ci.userid)
                       $query->joins
                 WHERE u.deleted = 0 AND ci.bloockcertid = :bloockcertid
                       $conditionssql
              ORDER BY $orderby";
    ...
    /**
     * Returns the total number of issues for a given bloockcert.
     *
     * @param int $bloockcertid
     * @param \stdClass $cm the course module
     ...
     */
    public static function get_number_of_issues($bloockcertid, $cm, $groupmode) {
        global $DB;
        ...
        // Add the conditional SQL and the bloockcertid to form all used parameters.
        $allparams = $conditionsparams + array('bloockcertid' => $bloockcertid);

        // Return the number of issues.
        $sql = "SELECT COUNT(u.id) as count
                  FROM {user} u
            INNER JOIN {bloockcert_issues} ci
                    ON u.id = ci.userid
                 WHERE u.deleted = 0
                   AND ci.bloockcertid = :bloockcertid
                       $conditionssql";
    ...
    public static function get_conditional_issues_sql($cm, $groupmode) {
        ...
        // Get all users that can manage this bloockcert to exclude them from the report.
        $context = \context_module::instance($cm->id);
        $conditionssql = '';
        $conditionsparams = array();

        // Get all users that can manage this certificate to exclude them from the report.
        $certmanagers = array_keys(get_users_by_capability($context, 'mod/bloockcert:manage', 'u.id'));
    ...
    public static function issue_certificate($certificateid, $userid, $recordid = null, $urlcert = null) {
        ...
        $issue = new \stdClass();
        $issue->userid = $userid;
        $issue->bloockcertid = $certificateid;
        if ($recordid !== null) {
        ...
        return $DB->insert_record('bloockcert_issues', $issue);
    }
    ...
    public static function generate_code() {
        ...
        while (!$uniquecodefound) {
            if (!$DB->record_exists('bloockcert_issues', array('code' => $code))) {
