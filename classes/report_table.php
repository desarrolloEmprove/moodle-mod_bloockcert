...
class report_table extends \table_sql {
    ...
    /**
     * @var int $bloockcertid The custom certificate id
     */
    protected $bloockcertid;
     ...
     */
    public function __construct($bloockcertid, $cm, $groupmode, $download = null) {
        parent::__construct('mod_bloockcert_report_table');
        ...
        $headers[] = get_string('receiveddate', 'bloockcert');
        $headers[] = get_string('code', 'bloockcert');

        // Check if we were passed a filename, which means we want to download it.
        if ($download) {
            $this->is_downloading($download, 'bloockcert-report');
        }
        ...
        if (!$this->is_downloading() && has_capability('mod/bloockcert:manage', $context)) {
            $columns[] = 'actions';
            $headers[] = '';
        }
        ...
        $this->bloockcertid = $bloockcertid;
        $this->cm = $cm;
    ...
    public function col_download($user) {
        ...
        $icon = new \pix_icon('download', get_string('download'), 'bloockcert');
        $link = new \moodle_url('/mod/bloockcert/view.php',
            [
                'id' => $this->cm->id,
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        $total = \mod_bloockcert\certificate::get_number_of_issues($this->bloockcertid, $this->cm, $this->groupmode);

        $this->pagesize($pagesize, $total);

        $this->rawdata = \mod_bloockcert\certificate::get_issues($this->bloockcertid, $this->groupmode, $this->cm,
