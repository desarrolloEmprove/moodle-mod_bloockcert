...
/**
 * The base class for the bloockcert elements.
 *
 ...
 * Class element
 *
 * All bloockcert element plugins are based on this class.
 *
 * @package    mod_bloockcert
 * @copyright  2024 Edwin Martinez <desarrollo@emprove.com.mx>
 * @copyright  2013 based on work by Mark Nelson <markn@moodle.com>
...
abstract class element {
    ...
    public function __construct($element) {
        $showposxy = get_config('bloockcert', 'showposxy');
    ...
    public function save_form_elements($data) {
        ...
        if (!empty($this->id)) { // Must be updating a record in the database.
            $element->id = $this->id;
            $return = $DB->update_record('bloockcert_elements', $element);

            \mod_bloockcert\event\element_updated::create_from_element($this)->trigger();

            return $return;
        } else { // Must be adding a new one.
            $element->element = $data->element;
            $element->pageid = $data->pageid;
            $element->sequence = \mod_bloockcert\element_helper::get_element_sequence($element->pageid);
            $element->timecreated = time();
            $element->id = $DB->insert_record('bloockcert_elements', $element, true);
            $this->id = $element->id;

            \mod_bloockcert\event\element_created::create_from_element($this)->trigger();
    ...
    public function delete() {
        ...
        $return = $DB->delete_records('bloockcert_elements', array('id' => $this->id));

        \mod_bloockcert\event\element_deleted::create_from_element($this)->trigger();
     ...
     *
     * @param \mod_bloockcert\edit_element_form $editelementform
     */
     ...
     *
     * @return \mod_bloockcert\edit_element_form
     */
