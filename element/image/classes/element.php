...
/**
 * This file contains the bloockcert element image's core interaction API.
 *
 * @package    bloockcertelement_image
...
namespace bloockcertelement_image;

/**
 * The bloockcert element image's core interaction API.
 *
 * @package    bloockcertelement_image
 * @copyright  2024 Edwin Martinez <desarrollo@emprove.com.mx>
 * @copyright  2013 based on work by Mark Nelson <markn@moodle.com>
...
class element extends \mod_bloockcert\element {
    ...
    public function render_form_elements($mform) {
        $mform->addElement('select', 'fileid', get_string('image', 'bloockcertelement_image'), self::get_images());

        \mod_bloockcert\element_helper::render_form_element_width($mform);

        \mod_bloockcert\element_helper::render_form_element_height($mform);
        ...
        $mform->addElement('select', 'alphachannel', get_string('alphachannel', 'bloockcertelement_image'), $alphachannelvalues);
        $mform->setType('alphachannel', PARAM_FLOAT);
        $mform->setDefault('alphachannel', 1);
        $mform->addHelpButton('alphachannel', 'alphachannel', 'bloockcertelement_image');

        if (get_config('bloockcert', 'showposxy')) {
            \mod_bloockcert\element_helper::render_form_element_position($mform);
        }
    ...
    public function validate_form_elements($data, $files) {
        ...
        // Validate the width.
        $errors += \mod_bloockcert\element_helper::validate_form_element_width($data);

        // Validate the height.
        $errors += \mod_bloockcert\element_helper::validate_form_element_height($data);

        // Validate the position.
        if (get_config('bloockcert', 'showposxy')) {
            $errors += \mod_bloockcert\element_helper::validate_form_element_position($data);
        }
    ...
    public function render_html() {
        ...
        // Get the image.
        $fs = get_file_storage();
        if ($file = $fs->get_file($imageinfo->contextid, 'mod_bloockcert', $imageinfo->filearea, $imageinfo->itemid,
                $imageinfo->filepath, $imageinfo->filename)) {
            $url = \moodle_url::make_pluginfile_url($file->get_contextid(), 'mod_bloockcert', 'image', $file->get_itemid(),
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'mod_bloockcert', 'image', 0, $this->filemanageroptions);
    ...
    public function after_restore($restore) {
        ...
        // Perform the update.
        $DB->set_field('bloockcert_elements', 'data', $elementinfo, array('id' => $this->get_id()));
    }
    ...
    public function get_file() {
        ...
        return $fs->get_file($imageinfo->contextid, 'mod_bloockcert', $imageinfo->filearea, $imageinfo->itemid,
            $imageinfo->filepath, $imageinfo->filename);
    }
    ...
    public static function get_images() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'mod_bloockcert', 'image', false, 'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = get_string('systemimage', 'bloockcertelement_image', $file->get_filename());
            }
        }
        // Loop through the files uploaded in the course context.
        if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'mod_bloockcert', 'image', false,
            'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = get_string('courseimage', 'bloockcertelement_image', $file->get_filename());
            }
        }

        \core_collator::asort($arrfiles);
        $arrfiles = array('0' => get_string('noimage', 'bloockcert')) + $arrfiles;
    ...
    public function copy_element($data) {
        ...
        if (isset($imagedata->filearea)) {
            ...
            $DB->set_field('bloockcert_elements', 'data', $this->save_unique_data($imagedata), ['id' => $this->get_id()]);
        }
