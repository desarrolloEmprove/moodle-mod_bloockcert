...
/**
 * This file contains the bloockcert element date's core interaction API.
 *
 * @package    bloockcertelement_date
...
namespace bloockcertelement_date;
 ...
 */
define('BLOOCKCERT_DATE_COURSE_GRADE', '0');

/**
 * Date - Issue
 */
define('BLOOCKCERT_DATE_ISSUE', '-1');

/**
 * Date - Completion
 */
define('BLOOCKCERT_DATE_COMPLETION', '-2');

/**
 * Date - Course start
 */
define('BLOOCKCERT_DATE_COURSE_START', '-3');

/**
 * Date - Course end
 */
define('BLOOCKCERT_DATE_COURSE_END', '-4');

/**
 * Date - Current date
 */
define('BLOOCKCERT_DATE_CURRENT_DATE', '-5');

/**
 * Date - Enrollment start
 */
define('BLOOCKCERT_DATE_ENROLMENT_START', '-6');

/**
 * Date - Entrollment end
 */
define('BLOOCKCERT_DATE_ENROLMENT_END', '-7');

/**
 * Date - Relative expiry date of 1 year
 */
define('BLOOCKCERT_DATE_EXPIRY_ONE', '-8');

/**
 * Date - Relative expiry date of 2 year
 */
define('BLOOCKCERT_DATE_EXPIRY_TWO', '-9');

/**
 * Date - Relative expiry date of 3 year
 */
define('BLOOCKCERT_DATE_EXPIRY_THREE', '-10');

/**
 * Date - Relative expiry date of 4 year
 */
define('BLOOCKCERT_DATE_EXPIRY_FOUR', '-11');

/**
 * Date - Relative expiry date of 5 year
 */
define('BLOOCKCERT_DATE_EXPIRY_FIVE', '-12');

require_once($CFG->dirroot . '/lib/grade/constants.php');

/**
 * The bloockcert element date's core interaction API.
 *
 * @package    bloockcertelement_date
 * @copyright  2024 Edwin Martinez <desarrollo@emprove.com.mx>
 * @copyright  2013 based on work by Mark Nelson <markn@moodle.com>
...
class element extends \mod_bloockcert\element {
    ...
    /**
     * This function renders the form elements when adding a bloockcert element.
     *
    ...
    public function render_form_elements($mform) {
        ...
        $dateoptions = array();
        $dateoptions[BLOOCKCERT_DATE_ISSUE] = get_string('issueddate', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_CURRENT_DATE] = get_string('currentdate', 'bloockcertelement_date');
        $completionenabled = $CFG->enablecompletion && ($COURSE->id == SITEID || $COURSE->enablecompletion);
        if ($completionenabled) {
            $dateoptions[BLOOCKCERT_DATE_COMPLETION] = get_string('completiondate', 'bloockcertelement_date');
        }
        $dateoptions[BLOOCKCERT_DATE_ENROLMENT_START] = get_string('enrolmentstartdate', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_ENROLMENT_END] = get_string('enrolmentenddate', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_EXPIRY_ONE] = get_string('expirydateone', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_EXPIRY_TWO] = get_string('expirydatetwo', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_EXPIRY_THREE] = get_string('expirydatethree', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_EXPIRY_FOUR] = get_string('expirydatefour', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_EXPIRY_FIVE] = get_string('expirydatefive', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_COURSE_START] = get_string('coursestartdate', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_COURSE_END] = get_string('courseenddate', 'bloockcertelement_date');
        $dateoptions[BLOOCKCERT_DATE_COURSE_GRADE] = get_string('coursegradedate', 'bloockcertelement_date');
        $dateoptions = $dateoptions + \mod_bloockcert\element_helper::get_grade_items($COURSE);

        $mform->addElement('select', 'dateitem', get_string('dateitem', 'bloockcertelement_date'), $dateoptions);
        $mform->addHelpButton('dateitem', 'dateitem', 'bloockcertelement_date');

        $mform->addElement('select', 'dateformat', get_string('dateformat', 'bloockcertelement_date'), self::get_date_formats());
        $mform->addHelpButton('dateformat', 'dateformat', 'bloockcertelement_date');
    ...
    public function render($pdf, $preview, $user) {
        ...
        $courseid = \mod_bloockcert\element_helper::get_courseid($this->id);
        ...
        if ($preview) {
            ...
            if ($dateitem == BLOOCKCERT_DATE_ISSUE) {
                $date = $issue->timecreated;
            } else if ($dateitem == BLOOCKCERT_DATE_EXPIRY_ONE) {
                $date = strtotime('+1 years', $issue->timecreated);
            } else if ($dateitem == BLOOCKCERT_DATE_EXPIRY_TWO) {
                $date = strtotime('+2 years', $issue->timecreated);
            } else if ($dateitem == BLOOCKCERT_DATE_EXPIRY_THREE) {
                $date = strtotime('+3 years', $issue->timecreated);
            } else if ($dateitem == BLOOCKCERT_DATE_EXPIRY_FOUR) {
                $date = strtotime('+4 years', $issue->timecreated);
            } else if ($dateitem == BLOOCKCERT_DATE_EXPIRY_FIVE) {
                $date = strtotime('+5 years', $issue->timecreated);
            } else if ($dateitem == BLOOCKCERT_DATE_CURRENT_DATE) {
                $date = time();
            } else if ($dateitem == BLOOCKCERT_DATE_COMPLETION) {
            ...
            } else if ($dateitem == BLOOCKCERT_DATE_ENROLMENT_START) {
            ...
            } else if ($dateitem == BLOOCKCERT_DATE_ENROLMENT_END) {
            ...
            } else if ($dateitem == BLOOCKCERT_DATE_COURSE_START) {
                $date = $DB->get_field('course', 'startdate', array('id' => $courseid));
            } else if ($dateitem == BLOOCKCERT_DATE_COURSE_END) {
                $date = $DB->get_field('course', 'enddate', array('id' => $courseid));
            } else {
                if ($dateitem == BLOOCKCERT_DATE_COURSE_GRADE) {
                    $grade = \mod_bloockcert\element_helper::get_course_grade_info(
                        $courseid,
                    ...
                    $grade = \mod_bloockcert\element_helper::get_grade_item_info(
                        $gradeitemid,
                        $dateitem,
                        $user->id
                    );
                } else {
                    $grade = \mod_bloockcert\element_helper::get_mod_grade_info(
                        $dateitem,
                        GRADE_DISPLAY_TYPE_DEFAULT,
                        $user->id
                    );
        ...
        if (!empty($date)) {
            \mod_bloockcert\element_helper::render_content($pdf, $this, $this->get_date_format_string($date, $dateformat));
        }
    }
    ...
    public function render_html() {
        ...
        return \mod_bloockcert\element_helper::render_html_content($this, $this->get_date_format_string(time(), $dateformat));
    }
     ...
     *
     * @param \restore_bloockcert_activity_task $restore
     */
    public function after_restore($restore) {
        ...
        if ($newitem = \restore_dbops::get_backup_ids_record($restore->get_restoreid(), 'course_module', $dateinfo->dateitem)) {
            ...
            $DB->set_field('bloockcert_elements', 'data', $this->save_unique_data($dateinfo), array('id' => $this->get_id()));
        }
    }
    ...
    protected static function get_ordinal_number_suffix($day) {
        if (!in_array(($day % 100), array(11, 12, 13))) {
            switch ($day % 10) {
                // Handle 1st, 2nd, 3rd.
                case 1:
                    return get_string('numbersuffix_st_as_in_first', 'bloockcertelement_date');
                case 2:
                    return get_string('numbersuffix_nd_as_in_second', 'bloockcertelement_date');
                case 3:
                    return get_string('numbersuffix_rd_as_in_third', 'bloockcertelement_date');
            }
        }
        return 'th';
    }
