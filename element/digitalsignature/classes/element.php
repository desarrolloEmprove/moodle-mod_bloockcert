...
/**
 * This file contains the bloockcert element digitial signature's core interaction API.
 *
 * @package    bloockcertelement_digitalsignature
...
namespace bloockcertelement_digitalsignature;

/**
 * The bloockcert element digital signature's core interaction API.
 *
 * @package    bloockcertelement_digitalsignature
 * @copyright  2024 Edwin Martínez <desarrollo@emprove.com.mx>
 * @copyright  2017 based on work by Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class element extends \bloockcertelement_image\element {
    ...
    public function render_form_elements($mform) {
        ...
        $mform->addElement('select', 'signaturefileid', get_string('digitalsignature', 'bloockcertelement_digitalsignature'),
            self::get_signatures());

        $mform->addElement('text', 'signaturename', get_string('signaturename', 'bloockcertelement_digitalsignature'));
        $mform->setType('signaturename', PARAM_TEXT);
        $mform->setDefault('signaturename', '');

        $mform->addElement('passwordunmask', 'signaturepassword',
            get_string('signaturepassword', 'bloockcertelement_digitalsignature'));
        $mform->setType('signaturepassword', PARAM_TEXT);
        $mform->setDefault('signaturepassword', '');

        $mform->addElement('text', 'signaturelocation', get_string('signaturelocation', 'bloockcertelement_digitalsignature'));
        $mform->setType('signaturelocation', PARAM_TEXT);
        $mform->setDefault('signaturelocation', '');

        $mform->addElement('text', 'signaturereason', get_string('signaturereason', 'bloockcertelement_digitalsignature'));
        $mform->setType('signaturereason', PARAM_TEXT);
        $mform->setDefault('signaturereason', '');

        $mform->addElement('text', 'signaturecontactinfo',
            get_string('signaturecontactinfo', 'bloockcertelement_digitalsignature'));
        ...
        $mform->addElement('filemanager', 'digitalsignature',
            get_string('uploaddigitalsignature', 'bloockcertelement_digitalsignature'), '',
            $this->signaturefilemanageroptions);
    }
    ...
    public function save_form_elements($data) {
        ...
        \mod_bloockcert\certificate::upload_files($data->digitalsignature, $context->id, 'signature');
    ...
    public function definition_after_data($mform) {
        ...
        file_prepare_draft_area($draftitemid, $context->id, 'mod_bloockcert', 'signature', 0,
            $this->signaturefilemanageroptions);
    ...
    public static function get_signatures() {
        ...
        if ($files = $fs->get_area_files(\context_system::instance()->id, 'mod_bloockcert', 'signature', false,
                'filename', false)) {
        ...
        if ($files = $fs->get_area_files(\context_course::instance($COURSE->id)->id, 'mod_bloockcert', 'signature', false,
                'filename', false)) {
            foreach ($files as $hash => $file) {
                $arrfiles[$file->get_id()] = $file->get_filename();
            }
        ...
        $arrfiles = array('0' => get_string('nosignature', 'bloockcertelement_digitalsignature')) + $arrfiles;
    ...
    public function get_signature_file() {
        ...
        return $fs->get_file($imageinfo->signaturecontextid, 'mod_bloockcert', $imageinfo->signaturefilearea,
