...
/**
 * bloockcert module upgrade code.
 *
 ...
 */
function xmldb_bloockcert_upgrade($oldversion) {
    global $DB;
    ...
    if ($oldversion < 2016120503) {
        ...
        $table = new xmldb_table('bloockcert_templates');
        $field = new xmldb_field('name', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, null, 'id');
        $dbman->change_field_precision($table, $field);

        // Savepoint reached.
        upgrade_mod_savepoint(true, 2016120503, 'bloockcert');
    }

    if ($oldversion < 2016120505) {
        $table = new xmldb_table('bloockcert');
        ...
        $table = new xmldb_table('bloockcert_issues');
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2016120505, 'bloockcert');
    }

    if ($oldversion < 2017050501) {
        // Remove any duplicate rows from bloockcert issue table.
        // This SQL fetches the id of those records which have duplicate bloockcert issues.
        // This doesn't return the first issue.
        $fromclause = "FROM (
                             SELECT min(id) AS minid, userid, bloockcertid
                               FROM {bloockcert_issues}
                           GROUP BY userid, bloockcertid
                            ) minid
                       JOIN {bloockcert_issues} ci
                         ON ci.userid = minid.userid
                        AND ci.bloockcertid = minid.bloockcertid
        ...
        if ($records = $DB->get_records_sql($getduplicatessql)) {
            ...
            $DB->delete_records_select('bloockcert_issues', "id IN ($ids)");
        }

        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050501, 'bloockcert');
    }
    ...
    if ($oldversion < 2017050502) {
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050502, 'bloockcert');
    }

    if ($oldversion < 2017050506) {
        $table = new xmldb_table('bloockcert_elements');
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2017050506, 'bloockcert');
    }
    ...
    if ($oldversion < 2018051705) {
        ...
        // Savepoint reached.
        upgrade_mod_savepoint(true, 2018051705, 'bloockcert');
    }
    ...
    if ($oldversion < 2019111803) {
        ...
        $key = new xmldb_key('templateid', XMLDB_KEY_FOREIGN, ['templateid'], 'bloockcert_templates', ['id']);
        $dbman->add_key($table, $key);

        $table = new xmldb_table('bloockcert_pages');
        $index = new xmldb_index('templateid', XMLDB_INDEX_UNIQUE, ['templateid']);
        if ($dbman->index_exists($table, $index)) {
            $dbman->drop_index($table, $index);
        }
        ...
        upgrade_mod_savepoint(true, 2019111803, 'bloockcert');
    }
    ...
    if ($oldversion < 2020110901) {
        ...
        upgrade_mod_savepoint(true, 2020110901, 'bloockcert');
    }
    ...
    if ($oldversion < 2021051702) {
        ...
        upgrade_mod_savepoint(true, 2021051702, 'bloockcert'); // Replace with the actual version number.
    }
    ...
    if ($oldversion < 2022041903) {
        ...
        upgrade_mod_savepoint(true, 2022041903, 'bloockcert'); // Replace with the actual version number.
    }
