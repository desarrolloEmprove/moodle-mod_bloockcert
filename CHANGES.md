...
Note - All hash comments refer to the issue number. Eg. #169 refers to https://github.com/mdjnelson/moodle-mod_bloockcert/issues/169.
...
- The 'element' field in the 'bloockcert_elements' table has been changed from a Text field to varchar(255) (#241).
...
## [3.3.9] - 2017-11-13
### Added
- Added capability ```mod/bloockcert:verifyallcertificates``` that provides a user with the ability to verify any certificate
  on the site by simply visiting the ```mod/bloockcert/verify_certificate.php``` page, rather than having to go to the
  verification link for each certificate.
- Added site setting ```bloockcert/verifyallcertificates``` which when enabled allows any person (including users not logged in)
...
## [3.3.6] - 2017-08-05
### Changed
- Renamed the column 'size' in the table 'bloockcert_elements' to 'fontsize' due to 'size' being a reserved word in Oracle.
