...
/**
 * Contains the class responsible for step definitions related to mod_bloockcert.
 *
...
/**
 * The class responsible for step definitions related to mod_bloockcert.
 *
 * @package mod_bloockcert
 * @category test
 * @copyright 2024 Edwin Martinez <desarrollo@emprove.com.mx>
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_bloockcert extends behat_base {
    ...
    public function i_add_the_element_to_the_certificate_template_page($elementname, $pagenum, $templatename) {
        ...
        $template = $DB->get_record('bloockcert_templates', array('name' => $templatename), '*', MUST_EXIST);
        $page = $DB->get_record('bloockcert_pages', array('templateid' => $template->id, 'sequence' => $pagenum),
            '*', MUST_EXIST);

        $this->execute('behat_forms::i_set_the_field_to', array($this->escape('element_' . $page->id),
            $this->escape($elementname)));
        $this->execute('behat_forms::press_button', get_string('addelement', 'bloockcert'));
    }
    ...
    public function i_verify_the_custom_certificate_for_user($certificatename, $username) {
        ...
        $certificate = $DB->get_record('bloockcert', array('name' => $certificatename), '*', MUST_EXIST);
        $user = $DB->get_record('user', array('username' => $username), '*', MUST_EXIST);
        $issue = $DB->get_record('bloockcert_issues', array('userid' => $user->id, 'bloockcertid' => $certificate->id),
            '*', MUST_EXIST);

        $this->execute('behat_forms::i_set_the_field_to', array(get_string('code', 'bloockcert'), $issue->code));
        $this->execute('behat_forms::press_button', get_string('verify', 'bloockcert'));
        $this->execute('behat_general::assert_page_contains_text', get_string('verified', 'bloockcert'));
        $this->execute('behat_general::assert_page_not_contains_text', get_string('notverified', 'bloockcert'));
    }
    ...
    public function i_can_not_verify_the_custom_certificate_for_user($certificatename, $username) {
        ...
        $this->execute('behat_forms::i_set_the_field_to', array(get_string('code', 'bloockcert'), $issue->code));
        $this->execute('behat_forms::press_button', get_string('verify', 'bloockcert'));
        $this->execute('behat_general::assert_page_contains_text', get_string('notverified', 'bloockcert'));
        $this->execute('behat_general::assert_page_not_contains_text', get_string('verified', 'bloockcert'));
    }
    ...
    public function i_visit_the_verification_url_for_custom_certificate($certificatename) {
        ...
        $certificate = $DB->get_record('bloockcert', array('name' => $certificatename), '*', MUST_EXIST);
        $template = $DB->get_record('bloockcert_templates', array('id' => $certificate->templateid), '*', MUST_EXIST);

        $url = new moodle_url('/mod/bloockcert/verify_certificate.php', array('contextid' => $template->contextid));
    ...
    public function i_visit_the_verification_url_for_the_site() {
        $url = new moodle_url('/mod/bloockcert/verify_certificate.php');
        $this->getSession()->visit($this->locate_path($url->out_as_local_url()));
    }
}
