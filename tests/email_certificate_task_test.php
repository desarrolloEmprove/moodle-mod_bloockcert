...
use mod_bloockcert\task\email_certificate_task;
 ...
 * @copyright  2017 based on work by Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \mod_bloockcert\task\email_certificate_task
 */
class email_certificate_task_test extends advanced_testcase {
     ...
     *
     * @covers \mod_bloockcert\task\email_certificate_task
     */
    public function test_email_certificates_no_elements() {
        ...
        $this->getDataGenerator()->create_module('bloockcert', ['course' => $course->id, 'emailstudents' => 1]);
    ...
    public function test_email_certificates_no_cap() {
        ...
        unassign_capability('mod/bloockcert:receiveissue', $roleids['student']);

        // Create a custom certificate.
        $bloockcert = $this->getDataGenerator()->create_module('bloockcert', ['course' => $course->id, 'emailstudents' => 1]);
    ...
    public function test_email_certificates_students() {
        ...
        \mod_bloockcert\certificate::issue_certificate($bloockcert->id, $user1->id);

        // Confirm there is only one entry in this table.
        $this->assertEquals(1, $DB->count_records('bloockcert_issues'));
        ...
        $issues = $DB->get_records('bloockcert_issues');
    ...
    public function test_email_certificates_others() {
        ...
        // Create a custom certificate.
        $bloockcert = $this->getDataGenerator()->create_module('bloockcert', array('course' => $course->id,
            'emailothers' => 'testbloockcert@example.com, doo@dah'));
        ...
        $this->assertEquals($CFG->noreplyaddress, $emails[0]->from);
        $this->assertEquals('testbloockcert@example.com', $emails[0]->to);

        $this->assertEquals($CFG->noreplyaddress, $emails[1]->from);
        $this->assertEquals('testbloockcert@example.com', $emails[1]->to);
    }
    ...
    public function test_email_certificates_students_not_visible() {
        ...
        assign_capability('mod/bloockcert:view', CAP_PROHIBIT, $roleids['student'], \context_course::instance($course->id));
    ...
    public function test_email_certificates_students_havent_met_required_time() {
        ...
        // Create a custom certificate.
        $bloockcert = $this->getDataGenerator()->create_module('bloockcert', array('course' => $course->id, 'emailstudents' => 1,
            'requiredtime' => '60'));
