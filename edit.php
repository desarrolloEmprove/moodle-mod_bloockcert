...
if ($tid) {
    ...
    // Set the page url.
    $pageurl = new moodle_url('/mod/bloockcert/edit.php', array('tid' => $tid));
} else { // Adding a new template.
    // Need to supply the contextid.
    $contextid = required_param('contextid', PARAM_INT);
    // Set the page url.
    $pageurl = new moodle_url('/mod/bloockcert/edit.php', array('contextid' => $contextid));
}
...
if ($context->contextlevel == CONTEXT_SYSTEM) {
    ...
    if (!$tid) {
        $PAGE->navbar->add(get_string('editbloockcert', 'bloockcert'));
    } else {
        $PAGE->navbar->add(get_string('editbloockcert', 'bloockcert'),
            new moodle_url('/mod/bloockcert/edit.php', ['tid' => $tid]));
    }
}
        ...
        switch ($action) {
            ...
            case 'deletepage' :
                if (!empty($confirm)) { // Check they have confirmed the deletion.
                    ...
                    $deleting = true;
                    // Create the message.
                    $message = get_string('deletepageconfirm', 'bloockcert');
                    // Create the link options.
                    $nourl = new moodle_url('/mod/bloockcert/edit.php', array('tid' => $tid));
                    $yesurl = new moodle_url('/mod/bloockcert/edit.php',
                        array(
                            'tid' => $tid,
                            'action' => 'deletepage',
                            'aid' => $actionid,
            ...
            case 'deleteelement' :
                if (!empty($confirm)) { // Check they have confirmed the deletion.
                    ...
                    $deleting = true;
                    // Create the message.
                    $message = get_string('deleteelementconfirm', 'bloockcert');
                    // Create the link options.
                    $nourl = new moodle_url('/mod/bloockcert/edit.php', array('tid' => $tid));
                    $yesurl = new moodle_url('/mod/bloockcert/edit.php',
                        array(
...
if ($tid) {
    $mform = new \mod_bloockcert\edit_form($pageurl, array('tid' => $tid));
    // Set the name for the form.
    $mform->set_data(array('name' => $template->get_name()));
} else {
    $mform = new \mod_bloockcert\edit_form($pageurl);
}

if ($data = $mform->get_data()) {
    // If there is no id, then we are creating a template.
    if (!$tid) {
        $template = \mod_bloockcert\template::create($data->name, $contextid);
    ...
    foreach ($data as $key => $value) {
        ...
        if (strpos($key, 'addelement_') !== false) {
            ...
            $url = new moodle_url('/mod/bloockcert/edit_element.php', $params);
            redirect($url);
        }
    }
    ...
    $url = new moodle_url('/mod/bloockcert/edit.php', array('tid' => $template->get_id()));
    redirect($url);
}
...
if ($tid && $context->contextlevel == CONTEXT_MODULE) {
    $loadtemplateurl = new moodle_url('/mod/bloockcert/load_template.php', array('tid' => $tid));
    $loadtemplateform = new \mod_bloockcert\load_template_form($loadtemplateurl, array('context' => $context), 'post',
